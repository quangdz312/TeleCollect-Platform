    ...
    <div className="space-y-3">
      <div className="relative aspect-[4/3] overflow-hidden rounded-lg border border-tech-border bg-tech-bg">
        ...
        <div className="absolute left-2 top-2">
          <Badge
            tone={state?.clutched ? "warn" : state?.detected ? "ok" : "neutral"}
          >
            {state?.clutched
              ? "CLUTCHED — đưa tay về tâm"
              : state?.detected
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          {starting ? "Starting…" : enabled ? "Stop camera" : "Start camera"}
        </Button>
        <Button
          variant="ghost"
          disabled={!enabled || !state?.detected}
          onClick={calibrate}
        >
          Calibrate
        </Button>
        ...
        >
          {active ? "Stop hand control" : "Activate hand control"}
        </Button>
      </div>
      <div className="grid grid-cols-3 gap-2 text-xs text-ink-400">
        ...
        <span className="font-mono">
          Forward/back {signed(state?.input.linear[0])}
        </span>
        <span className="font-mono">
          Left/right {signed(state?.input.linear[1])}
        </span>
        <span className="font-mono">
          Up/down {signed(state?.input.linear[2])}
        </span>
        <span className="font-mono">
          Xoay Z {signed(state?.input.angular[2])}
        </span>
        <span className="font-mono">
          {state?.rollVelocityDeg.toFixed(1) ?? "0.0"}°/s
        </span>
      ...
      <div className="rounded-md border border-ink-700/60 bg-ink-850/60 p-2 text-[11px] text-ink-400">
        <div className="mb-1 font-semibold uppercase tracking-wider text-ink-300">
          Hand latency diagnostics
        </div>
        <div className="grid grid-cols-2 gap-x-3 gap-y-1">
          <span>MediaPipe p50</span>
          <span className="text-right font-mono">
            {ms(diagnostics?.inferenceP50)}
          </span>
          <span>MediaPipe p95</span>
          <span className="text-right font-mono">
            {ms(diagnostics?.inferenceP95)}
          </span>
          <span>Gesture mapping p50</span>
          <span className="text-right font-mono">
            {ms(diagnostics?.mappingP50)}
          </span>
          <span>Camera frame interval</span>
          <span className="text-right font-mono">
            {ms(diagnostics?.frameIntervalP50)}
          </span>
          <span>Tracking rate</span>
          <span className="text-right font-mono">
            {diagnostics ? `${diagnostics.cameraFps.toFixed(1)} fps` : "—"}
          </span>
        </div>
      </div>
      <p className="text-xs text-ink-400">
        Move your hand to drive XYZ. Turn your wrist left or right to change the
        palm direction and rotate the gripper about Z; stop turning and the
        gripper stops. Open hand: open the gripper. Fist: close it. Hold 👍 to
        clutch.
      </p>
    </div>
  );
...
function pushSample(samples: number[], value: number) {
  ...
  if (samples.length > SAMPLE_WINDOW)
    samples.splice(0, samples.length - SAMPLE_WINDOW);
}
