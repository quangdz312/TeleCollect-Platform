...
export function ScriptedReviewDetail({ episodeId, returnTo }: { episodeId: string; returnTo: string }) {
  ...
  const submit = async (decision: "approved" | "rejected") => {
    if (decision === "rejected" && !note.trim()) { setError("Rejecting a scripted episode needs a reason in the note."); return; }
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><a href={returnTo} className="text-xs text-accent-400 hover:underline">← Review queue</a><h1 className="mt-1 font-heading text-[22px] font-bold tracking-tight">{episode.task}</h1><p className="text-sm text-ink-400">{episode.display_name || episode.demo} · {episode.length} frames · scripted collection</p></div><div className="flex flex-wrap gap-2"><Badge tone="info">Scripted</Badge><AutoLabelBadge label={episode.auto_label} reason={episode.auto_label_reason} /><Badge tone={episode.label ? (episode.label.human_decision === "approved" ? "ok" : "bad") : "warn"}>{episode.label?.human_decision ?? (episode.audit_required ? "audit required" : "needs review")}</Badge>{episode.label?.decision_source === "auto_gate" && <Badge tone="info">auto gate</Badge>}</div></div>
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]"><Card title="Playback">{videoReady ? <><video src={labeling.videoUrl(episodeId)} controls className="w-full rounded-lg border border-tech-border bg-tech-bg" /><TrimBar episode={episode} /></> : <Empty>The video is still rendering. Hit refresh in a few seconds.</Empty>}</Card><div className="space-y-5"><Card title="Verdict"><div className="space-y-3"><div className="flex gap-2"><Badge tone="info">Scripted</Badge><Badge>{episode.task}</Badge><Badge>{episode.length} frames</Badge></div>{episode.audit_required && <Alert tone="info">This episode qualified for an auto-pass but was held back in the audit sample. Watch it and decide as usual.</Alert>}{episode.label?.decision_source === "auto_gate" && <Alert tone="info">Decided automatically: {episode.label.gate_reason || episode.auto_gate_reason}. You can override it with the buttons below.</Alert>}<Field label="Reviewer notes"><TextArea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Note / reason for rejecting" /></Field><div className="grid grid-cols-2 gap-2"><Button variant="success" onClick={() => void submit("approved")}>Accept</Button><Button variant="danger" onClick={() => void submit("rejected")}>Reject</Button></div><Button variant="subtle" className="w-full" onClick={() => void load()}>Refresh video</Button>{error && <Alert>{error}</Alert>}{saved && <Alert tone="ok">{saved}</Alert>}</div></Card><Card title="Capture format"><dl className="space-y-1.5 text-xs"><Row label="Source" value="Scripted" /><Row label="Resolution" value="960 × 640 (3 panes)" /><Row label="Codec" value="H.264 / yuv420p" /><Row label="Frames" value={String(episode.length)} /></dl></Card></div></div>
  </div>;
}
