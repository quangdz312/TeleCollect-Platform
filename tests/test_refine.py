...
def teleop_paths():
    ...
    for directory in TELEOP_DIRS:
        # A session that dropped before the recorder closed leaves the episode
        # directory behind with no trajectory in it.
        actions = Path(directory) / "actions.parquet"
        if not actions.is_file():
            continue
        table = pq.read_table(actions, columns=["ee_pose"])
