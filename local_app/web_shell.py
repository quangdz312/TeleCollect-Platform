    ...
    return rf"""
      (() => {{
        ...
        const openLibrary = async () => {{
          ...
          episodes.forEach((item)=>{{const row=document.createElement('tr');row.style.borderTop='1px solid #e2e8f0'; const status=['unreviewed','accepted','rejected','recollect'].map(s=>`<option ${{item.status===s?'selected':''}}>${{s}}</option>`).join(''); row.innerHTML=`<td>${{item.title}}</td><td>${{item.task}}</td><td align="center">${{item.frames}}</td><td><select>${{status}}</select></td><td align="center">${{item.complete_for_lerobot?'ready':'incomplete'}}</td><td>${{item.tags.join(', ')}}</td>`; const select=row.querySelector('select'); select.onchange=async()=>{{await fetch('/api/v1/local/library/episodes/'+encodeURIComponent(item.id),{{method:'PATCH',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{status:select.value}})}})}}; const actions=document.createElement('td'); const edit=document.createElement('button');edit.textContent='Edit';edit.onclick=async()=>{{const tags=prompt('Tags (comma separated)',item.tags.join(','));const note=prompt('Note',item.note);if(tags!==null)await fetch('/api/v1/local/library/episodes/'+encodeURIComponent(item.id),{{method:'PATCH',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{tags:tags.split(','),note:note??item.note}})}});openLibrary();}}; const trash=document.createElement('button');trash.textContent='Trash';trash.onclick=async()=>{{await fetch('/api/v1/local/library/episodes/'+encodeURIComponent(item.id),{{method:'PATCH',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{trashed:true}})}});openLibrary();}};actions.append(edit,' ',trash);row.appendChild(actions);body.appendChild(row);}});table.appendChild(body);card.appendChild(table);
        }};
        ...
        const openEpisode = async (episode) => {{
          ...
          const facts=document.createElement('p');facts.style.cssText='font-size:13px;color:#475569';facts.textContent=`Task: ${{episode.task}} • ${{episode.frames}} frames • cameras: ${{episode.complete_for_lerobot?'ready':'incomplete'}} • ID: ${{episode.id}}`;card.appendChild(facts);
